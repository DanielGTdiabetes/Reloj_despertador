...
def _draw_solar_arc(draw: ImageDraw.Draw, period: str, progress: float,
    ...
    # Track base (muy sutil, solo como guía)
...
class RoundHomeScreen:
    ...
    def render(self, now, weather: dict, sun_info: dict,
        ...
        _text_center(draw, 22, date_str, F.date_top, pal["a"])

        # Hora
        t_str = now.strftime("%H:%M")
        bb    = draw.textbbox((0, 0), t_str, font=F.clock)
        draw.text(((W-(bb[2]-bb[0]))//2, 42), t_str, font=F.clock, fill=WHITE)

        # Temp min/max (debajo de la hora, bien separado)
        temp_max = weather.get("temp_max")
        temp_min = weather.get("temp_min")
        if temp_max is not None and temp_min is not None:
            minmax = f"min {temp_min:.0f}  max {temp_max:.0f}"
            _text_center(draw, 108, minmax, F.weather_sub, DIM_WHITE)

        # Icono meteorológico PIL — más grande
        desc = (weather.get("description") or "").upper()
        draw_weather_icon(img, desc, CX, 142, size=60)

        # Descripción (truncada)
        if desc:
            _text_center(draw, 174, desc[:20], F.weather_sub, DIM_WHITE)

        # Temperatura actual
        temp = weather.get("temp")
        temp_str = f"{temp:.1f}\u00b0C" if temp is not None else "--\u00b0C"
        _text_center(draw, 188, temp_str, F.temp_big, pal["a"])
